...
ponder.on("LattePoolA:Borrowed", async ({ event, context }) => {
  // insert solver
  const solverFound = await context.db.find(solver, { id: event.args.user });
  if (!solverFound) {
    await context.db.insert(solver).values({
...
ponder.on("LattePoolA:Repaid", async ({ event, context }) => {
  // insert solver
  const solverFound = await context.db.find(solver, { id: event.args.user });
  if (!solverFound) {
    await context.db.insert(solver).values({
...
ponder.on("LattePoolB:Borrowed", async ({ event, context }) => {
  // insert solver
  const solverFound = await context.db.find(solver, { id: event.args.user });
  if (!solverFound) {
    await context.db.insert(solver).values({
...
ponder.on("LattePoolB:Repaid", async ({ event, context }) => {
  // insert solver
  const solverFound = await context.db.find(solver, { id: event.args.user });
  if (!solverFound) {
    await context.db.insert(solver).values({
